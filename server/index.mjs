...
app.listen(port,host,()=>console.log(`VANTA: http://${host}:${port} (${store.type})`));
if(process.env.AUTO_CONFIGURE_BOT==='1'){
 let attempts=0;
 const setup=async()=>{try{const username=await configureBot({token:process.env.BOT_TOKEN,publicUrl:process.env.PUBLIC_URL||process.env.RENDER_EXTERNAL_URL,secret:process.env.WEBHOOK_SECRET});process.env.BOT_USERNAME=username;console.log(`Бот @${username} подключён.`);}catch{console.error('Bot setup failed. Check BOT_TOKEN, WEBHOOK_SECRET and PUBLIC_URL; retry with npm run bot:setup.');if(++attempts<3)setTimeout(setup,15000).unref();}};
 setup();
}
